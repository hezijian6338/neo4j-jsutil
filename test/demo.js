class Demo {
  ...
  async test() {
    ...
    try {
      ...
      console.log(
        'Matched created node with name: ' + createdNodeId.properties.name
      )
  ...
  async test2() {
    ...
    const q = `match (a:${nodeLabel} ${JSON.stringify(nodeValue).replace(
      /"([^(")"]+)":/g,
      '$1:'
    )}), (b:${relateNodeLabel} ${JSON.stringify(relateNodeValue).replace(
      /"([^(")"]+)":/g,
      '$1:'
    )}) merge (a)-[r:${relationName} ${JSON.stringify(relationValue).replace(
      /"([^(")"]+)":/g,
      '$1:'
    )}]->(b) return a,r,b`

    try {
      const result = await this.session.run(q)
